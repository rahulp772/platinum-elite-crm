...
export class TransactionsController {
  ...
  async findAll(@Request() req: { user: User }) {
    const transactions = await this.transactionsService.findAll(req.user.tenantId);
    return { data: transactions, success: true };
  }

  @Get(':id')
  async findOne(@Param('id') id: string, @Request() req: { user: User }) {
    const transaction = await this.transactionsService.findOne(id, req.user.tenantId);
